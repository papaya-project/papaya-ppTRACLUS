...
share* buildmatrix(share *s_x1_b,share *s_x1_e,
  ...
  /*
  share* ss_x2_b= bc-> PutRepeaterGate(nvals,bc->PutSplitterGate(bc->PutCombineAtPosGate(s_x1_b,i)));
  share* ss_x2_e= bc-> PutRepeaterGate(nvals,bc->PutSplitterGate(bc->PutCombineAtPosGate(s_x1_e,i)));
  share* ss_y2_b= bc-> PutRepeaterGate(nvals,bc->PutSplitterGate(bc->PutCombineAtPosGate(s_y1_b,i)));
  share* ss_y2_e= bc-> PutRepeaterGate(nvals,bc->PutSplitterGate(bc->PutCombineAtPosGate(s_y1_e,i)));*/
  s_out = BuildSquaredEuclidDistanceCircuit(s_x1_b, s_x1_e, s_x1_b_2, s_x1_e_2,
    s_y1_b, s_y1_e, s_y1_b_2, s_y1_e_2, (BooleanCircuit*) bc);
...
vector<VerificationLineSegment> BuildClusterUsingYao(vector<uint32_t*> ps_out, uint32_t nvals, share* minLns
  , BooleanCircuit *yc,BooleanCircuit *bc, uint32_t itera, ABYParty* party)
 {
  ...
  for (uint32_t i=0;i<nvals;i++){
    s_out[i]=bc->PutSharedSIMDINGate(nvals,ps_out[i],8);
  }
...
share* construct(e_role role, const string& address, uint16_t port, seclvl seclvl, uint32_t nthreads,
  ...
  uint64_t* x1_b = new uint64_t[nvals*nvals];
  uint64_t* x1_e = new uint64_t[nvals*nvals];
  uint64_t* y1_b = new uint64_t[nvals*nvals];
  uint64_t* y1_e = new uint64_t[nvals*nvals];

  uint64_t* x1_b_2 = new uint64_t[nvals*nvals];
  uint64_t* x1_e_2 = new uint64_t[nvals*nvals];
  uint64_t* y1_b_2 = new uint64_t[nvals*nvals];
  uint64_t* y1_e_2 = new uint64_t[nvals*nvals];

  share *s_x1_b, *s_x1_e, *s_y1_b, *s_y1_e, *s_out, *s_minLns,*s_x1_b_2, *s_x1_e_2, *s_y1_b_2, *s_y1_e_2;
  ...
  uint32_t counter=0;
  for(int i = 0; i < nvals*nvals; ++i) {
    x1_b[i] = linesegments[i%nvals][0];
    y1_b[i] = linesegments[i%nvals][1];
    x1_e[i] = linesegments[i%nvals][2];
    y1_e[i] = linesegments[i%nvals][3];
    x1_b_2[i] = linesegments[counter][0];
    y1_b_2[i] = linesegments[counter][1];
    x1_e_2[i] = linesegments[counter][2];
    y1_e_2[i] = linesegments[counter][3];
    if(i!=0 && i%nvals==0){
      counter++;
    }
  }
  ...
  //for (uint32_t i=0;i<nvals;i++){
  s_x1_b=bc->PutSharedSIMDINGate(nvals*nvals,x1_b,64);
  s_x1_e=bc->PutSharedSIMDINGate(nvals*nvals,y1_b,64);
  s_y1_b=bc->PutSharedSIMDINGate(nvals*nvals,x1_e,64);
  s_y1_e=bc->PutSharedSIMDINGate(nvals*nvals,y1_e,64);
  s_x1_b_2 = bc->PutSharedSIMDINGate(nvals*nvals,x1_b_2, 64);
  s_y1_b_2 = bc->PutSharedSIMDINGate(nvals*nvals,y1_b_2, 64);
  s_x1_e_2 = bc->PutSharedSIMDINGate(nvals*nvals,x1_e_2, 64);
  s_y1_e_2 = bc->PutSharedSIMDINGate(nvals*nvals,y1_e_2, 64);
  s_threshold=bc-> PutRepeaterGate(nvals*nvals,bc->PutSharedINGate(threshold,64));
  s_minLns = bc->PutSharedINGate(minLns,32);


  bc->PutPrintValueGate(s_x1_b,"s_x1_b");
  bc->PutPrintValueGate(s_x1_b_2,"s_x1_b_2");
 
  s_out =buildmatrix(s_x1_b,s_x1_e,s_y1_b, s_y1_e,s_x1_b_2,s_x1_e_2,s_y1_b_2,s_y1_e_2, (BooleanCircuit*) bc ,nvals,s_threshold);
  /*
